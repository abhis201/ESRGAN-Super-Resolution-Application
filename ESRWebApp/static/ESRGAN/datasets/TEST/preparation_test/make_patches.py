        ...
        for i in range(num_row):
            ...
            for j in range(num_col):
                if (j+1)*image_size > width:
                    break
                cv2.imwrite(os.path.join(path, f'{image_name.split(".")[0]}_{str(image_index).zfill(4)}.png'), img[i*image_size:(i+1)*image_size, j*image_size:(j+1)*image_size])
                image_index += 1
        df = pd.DataFrame({'sr_no' : list_index, 'height' : list_height, 'width' : list_width})
        print(df)
        df.to_csv(f'{path}_metadata.csv', index = False)
